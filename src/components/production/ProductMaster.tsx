...
export default function ProductMaster() {
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-24">
      ...
      <div className="sticky top-0 lg:top-0 z-50 bg-background/95 backdrop-blur-md py-6 mb-8 border-b-2 border-black/10 -mx-8 px-8 shadow-md">
        <div className="flex flex-col md:flex-row gap-6 max-w-7xl mx-auto w-full">
          <div className="flex-1 relative group/field">
            ...
            <input
              ...
              className="w-full pl-16 pr-6 h-16 bg-white border-[2.5px] border-black rounded-xl font-bold text-black focus:outline-none focus:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all placeholder:text-black/20 uppercase tracking-tight"
            />
          </div>
        ...
        <div className="flex flex-col gap-4">
          {filteredProducts.map((p) => (
            <div
              ...
              className="group relative bg-white border-[2.5px] border-black rounded-2xl p-4 md:p-6 hover:bg-[#F3E8FF]/20 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer flex flex-col md:flex-row md:items-center gap-6 active:translate-x-[2px] active:translate-y-[2px] active:shadow-none"
            >
              {/* Product Icon & Info Primary */}
              <div className="flex items-center gap-6 flex-1 min-w-0">
                <div className="w-16 h-16 bg-neo-purple/10 border-[2.5px] border-black rounded-xl flex items-center justify-center shrink-0 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] group-hover:bg-neo-purple transition-colors">
                  <Package size={32} className="text-black" />
                </div>

                <div className="min-w-0 flex-1">
                  <p className="text-[10px] text-black/40 font-black uppercase tracking-[0.2em] italic mb-1 truncate">
                    {p.sku || 'NO-REF'}
                  </p>
                  <h3 className="text-lg font-black text-black tracking-tight uppercase italic truncate leading-tight">
              ...
              {/* Metrics Section */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-12 md:px-12 border-t md:border-t-0 md:border-l-[2.5px] border-black/10 pt-4 md:pt-0">
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-black/40 uppercase tracking-widest leading-none">Thời gian SX</p>
                  <div className="flex items-center gap-2">
                    <p className="font-black text-black tabular-nums text-base italic">
                      {p.productionTimeStd || 0}<span className="text-[10px] ml-1">min</span>
                    </p>
                  </div>
                </div>

                <div className="space-y-1">
                  <p className="text-[9px] font-black text-black/40 uppercase tracking-widest leading-none">Giá bán sỉ</p>
                  <div className="flex items-center gap-2">
                    <p className="font-black text-black tabular-nums text-base italic truncate">
                      {Number(p.wholesalePrice || 0).toLocaleString()}<span className="text-[10px] ml-1">đ</span>
                    </p>
                  </div>
                </div>

                <div className="hidden md:flex flex-col justify-center space-y-1">
                  <div className="flex items-center gap-2">
                    <div className={cn(
                      "w-8 h-8 rounded-lg border-[2px] border-black flex items-center justify-center shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]",
                      p.versionCount && p.versionCount > 1 ? "bg-neo-purple" : "bg-white"
                    )}>
                      <p className="text-[11px] font-black">{p.versionCount || 0}</p>
                    </div>
                    <span className="text-[9px] font-black text-black/30 uppercase tracking-widest">BOM</span>
                  </div>
                </div>
              </div>

              {/* Action Buttons */}
              <div className="flex items-center justify-end gap-3 shrink-0 pt-4 md:pt-0 border-t md:border-t-0 border-black/10">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleEditProduct(p);
                  }}
                  className="w-11 h-11 rounded-xl bg-white text-black flex items-center justify-center hover:bg-neo-yellow transition-all border-[2px] border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none"
                >
                ...
                <button
                  ...
                  className="w-11 h-11 bg-red-50 text-red-500 border-[2px] border-black rounded-xl hover:bg-red-500 hover:text-white transition-all shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none flex items-center justify-center"
                >
                  <Trash2 size={18} strokeWidth={3} />
                </button>

                <div className="w-11 h-11 rounded-xl bg-black text-white flex items-center justify-center hover:bg-neo-purple hover:text-black transition-all border-[2px] border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]">
                  <ArrowUpRight size={20} strokeWidth={3} />
                </div>
              </div>

              {/* Status Indicator (Mobile Float) */}
